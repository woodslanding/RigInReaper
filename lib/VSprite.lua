...
--
-- The VSprite class simplifies a number of common image use-cases, such as
...
-- right, starting at 0.
function VSprite:draw(x, y, frame, horizontal)
  ...
  local srcX, srcY = self:getFrame(frame, horizontal)
  local srcW, srcH
...
-- Defaults to a vertical set of frames.  Set horizontal to true for horizontal frames
function VSprite:getFrame(frame,horizontal)
  if self.frame.w == 0 or self.frame.h == 0 or not frame then return 0, 0 end
  if horizontal then return frame * self.frame.w, 0
                else return 0, frame * self.frame.h 
  end
end
