...
function Bank:addPreset(name, remove)
    if not remove then
        self.presets[name]
        M.Msg('inserted preset: '..name)
    else table.remove(self.presets,name)
    end
end
...
function Plugin:setBankPreset(bankName,preset,remove)
    if not self.banks[bankName] then print('non-existent bank: '..bankName) return end
    M.Msg('setting bank preset '..preset..', remove = ',remove)
    self.banks[bankName]:addPreset(preset,remove)
end
